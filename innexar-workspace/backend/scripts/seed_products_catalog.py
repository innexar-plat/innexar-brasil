...
async def main() -> None:
    ...
    async with engine.begin() as conn:
        ...
        # setval(..., 0) is invalid in PostgreSQL; use at least 1.
        await conn.execute(
            text(
                "SELECT setval(pg_get_serial_sequence('billing_products', 'id'), "
                "GREATEST((SELECT COALESCE(MAX(id), 0) FROM billing_products), 1))"
            )
        )
        await conn.execute(
            text(
                "SELECT setval(pg_get_serial_sequence('billing_price_plans', 'id'), "
                "GREATEST((SELECT COALESCE(MAX(id), 0) FROM billing_price_plans), 1))"
            )
        )
