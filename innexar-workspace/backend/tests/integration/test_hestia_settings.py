...
async def test_hestia_settings_create_and_read(db_session: AsyncSession) -> None:
    ...
    r = await db_session.execute(select(HestiaSettings).where(HestiaSettings.org_id == "test-org").limit(1))
...
async def test_hestia_settings_update(db_session: AsyncSession) -> None:
    ...
    settings = HestiaSettings(org_id="org2", grace_period_days=5, auto_suspend_enabled=True)
    db_session.add(settings)
    await db_session.flush()
