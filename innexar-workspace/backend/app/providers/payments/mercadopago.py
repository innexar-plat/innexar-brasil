...
class MercadoPagoProvider:
    ...
    def handle_webhook(self, body: bytes, headers: dict[str, str]) -> WebhookResult:
        ...
        if not self._access_token:
            # Keep a stable 401-like message for callers/tests that treat missing token as unauthorized.
            return WebhookResult(processed=False, message="Failed to get payment: 401")
        try:
